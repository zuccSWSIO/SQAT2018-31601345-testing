...
public class MaintainServiceTypeInterFrm extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MaintainServiceTypeInterFrm frame = new MaintainServiceTypeInterFrm();
	...
	/*用于填充表格*/
	private void fillTable(Service_type goodType, boolean hasId) {//boolean hadId用于配合Dao类中的万能查询方法使用（当要查询的数据中有int类型的数据时方便区别是默认值0还是搜索值是0的情况）
		DefaultTableModel dtm = (DefaultTableModel) jtb_table.getModel();
		dtm.setRowCount(0);//设置表格行数为零，即在下次查询前清空表格。
		/*将storeXXX赋值为空并将各个文本框赋值为空*/
		storeId = 0;
		storeName = null;
		storeDetail = null;
		jtf_id.setText("");
		...
		try {
			...
			ResultSet rs = serviceTypeDao.serviceTypeList(con, goodType, hasId);//查询数据库并得到结果集合ResultSet
			...
			while(rs.next()) {
				...
				v.add(rs.getString("service_type_id"));
				v.add(rs.getString("service_type_name"));
				v.add(rs.getString("service_type_detail"));
				dtm.addRow(v);
			}
...
//		}
		...
		if(StringUtil.isEmpty(jtf_name.getText())) {
			...
			result = false;
			JOptionPane.showMessageDialog(null, "请填写新的服务类型名称");
			return result;
		}
		if(StringUtil.isEmpty(jta_detail.getText())) {
			result = false;
			JOptionPane.showMessageDialog(null, "请填写新的服务类别详情");
			return result;
		}
		
		/*③用户再次确认是否要更新该条记录*/
		int choice = -1;
		choice = JOptionPane.showConfirmDialog(null, "确定要更新这条服务类别信息吗？\n服务类别编号：" + storeId + "\n服务类别名称：" + storeName + " ——> " + jtf_name.getText() + "\n服务类别详情：" + storeDetail + " ——> " + jta_detail.getText());
		if(choice != 0) {
			result = false;
			return result;
		}
		...
		/*查询是否有重名的服务类别*/
		Connection con = null;
		Service_type temp = new Service_type();
		temp.setService_type_name(jtf_name.getText());
		
		try {
			con = dbUtil.getCon();
			boolean  hasEqual = serviceTypeDao.isEqual(con, temp);
			if(hasEqual) {
				result = false;
				JOptionPane.showMessageDialog(null, "已经存在相同名称的服务类别，更新失败");
				jtf_name.setText(storeName);
				return result;
			}
	...
	private boolean allowDelete() {
		...
		int choice = -1;
		choice = JOptionPane.showConfirmDialog(null, "是否要删除这条服务类型？\n服务类型编号：" + storeId + "\n服务类型名称：" + storeName + "\n服务类型详情：" + storeDetail);
		if(choice != 0) {
			result = false;
			return result;
		}
	 ...
	 * Create the frame.
	 */
	public MaintainServiceTypeInterFrm() {
		setResizable(true);
		setIconifiable(true);
		setMaximizable(true);
		setTitle("\u670D\u52A1\u7C7B\u522B\u4FE1\u606F\u7EF4\u62A4");
		...
		jb_search.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				/*实现全局、按服务种类编号、按服务种类名称、按服务种类详情模糊查找*/
				...
				if(StringUtil.isEmpty(temp)) {
					...
					if(IntegerUtil.isNumeric(temp)) {
						serviceType.setService_type_id(Integer.parseInt(temp));
						hasId = true;
					}
					serviceType.setService_type_name(temp);
					serviceType.setService_type_detail(temp);
				}
				else if(jrb_id.isSelected()) {//实现按服务种类编号搜索（模糊）
					if(!IntegerUtil.isNumeric(temp)) {
						JOptionPane.showMessageDialog(null, "服务种类编号应为纯数字，请重新输入");
						return;
					}
					serviceType.setService_type_id(Integer.parseInt(temp));
					hasId = true;
				}
				else if(jrb_name.isSelected()) {//实现按服务种类名称搜索（模糊）
					serviceType.setService_type_name(temp);
				}
				else if(jrb_detail.isSelected()){//实现按服务种类描述查询
					serviceType.setService_type_detail(temp);
				}
				
				//执行搜索
				fillTable(serviceType, hasId);
			}
		});
		...
		JLabel label = new JLabel("\u670D\u52A1\u79CD\u7C7B\u7F16\u53F7\uFF1A");
		...
		JLabel lblNewLabel_1 = new JLabel("\u670D\u52A1\u79CD\u7C7B\u540D\u79F0\uFF1A");
...
/*实现更新功能*/
				...
				/*更新数据库并刷新表格*/
				Connection con = null;
				Service_type serviceType = new Service_type();//实体model类
				serviceType.setService_type_id(storeId);//set所有有变动的数据
				serviceType.setService_type_name(jtf_name.getText());
				serviceType.setService_type_detail(jta_detail.getText());
				
				try {
					con = dbUtil.getCon();
					int temp = serviceTypeDao.updateServiceType(con, serviceType);
					if(temp == 1) {
						JOptionPane.showMessageDialog(null, "该服务类别信息更新成功");
						//刷新表格（完整列出，不能恢复之前的搜索结果）
						fillTable(new Service_type(), false);
						//更新storeXXX
						storeId = Integer.parseInt(jtf_id.getText());
						storeName = jtf_name.getText();
						storeDetail = jta_detail.getText();
		...
		jb_delete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				Connection con = null;
				Service_type serviceType = new Service_type();//new一个实体类
				serviceType.setService_type_id(storeId);//设置实体类的查询条件
				try {
					con = dbUtil.getCon();
					int temp = serviceTypeDao.deleteServiceType(con, serviceType);//变量temp用于接受是否删除成功的信息，如果返回1则删除成功
					if(temp == 1) {
